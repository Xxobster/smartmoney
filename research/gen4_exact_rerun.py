"""Re-run Gen4 EXACTLY like run_gen2 evaluate_outer_oos (full-history features)."""
...
from paths import CONFIG_DIR, DATASETS
...
def evaluate_window(feat, spec, symbol, start_ms, end_ms):
    ...
    if len(sub) < 50:
        return {"n_trades": 0, "net_pnl": 0.0, "profit_factor": float("nan")}
    ...
    return run_tradesim_on_features(
        ...
        funding_ts_ms=fts,
        funding_rate=fr,
        strategy_id=f"recon_exact_{symbol}",
...
def main() -> None:
    ...
    print("Precomputing full-history features (same as Gen4 OOS)...")
    cache = {
    ...
    out = {"reported_net": oos["net_pnl"], "reported_pf": oos["pooled_pf"], "folds": [], "stitched": []}
    ...
    for sym in symbols:
        ...
        m = run_tradesim_on_features(
            ...
            funding_ts_ms=fts,
            funding_rate=fr,
            strategy_id=f"recon_stitch_{sym}",
            print_headline=True,
        )
        out["stitched"].append(
            {
    ...
    # last 4m with full-history features
    ...
    for sym in symbols:
        ...
        print(f"last4m {sym}: n={m.get('n_trades')} net={m.get('net_pnl'):.2f} pf={m.get('profit_factor')}")

    out["last_4m"] = {
        "n": n4,
        "net": net4,
        "pf": (gp4 / gl4) if gl4 > 0 else None,
    }
    out["stitched_pooled"] = {
        "n": n,
        "net": net,
    ...
    Path("artifacts/reports/gen4_exact_rerun.json").write_text(json.dumps(out, indent=2, default=str), encoding="utf-8")
    print(json.dumps(out["stitched_pooled"], indent=2))
    print("last4m pooled", out["last_4m"])
    print("wrote artifacts/reports/gen4_exact_rerun.json")
