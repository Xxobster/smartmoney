"""Elder Impulse System. Vectorized EMA 13 + MACD histogram 12/26/9."""
...
ATR_STOP = 2.0
RR = 1.5
WARMUP = MACD_SLOW + MACD_SIG + EMA_N + ATR_N
...
def build_signal_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    macd = (
        pd.Series(c).ewm(span=MACD_FAST, adjust=False, min_periods=MACD_FAST).mean()
        - pd.Series(c).ewm(span=MACD_SLOW, adjust=False, min_periods=MACD_SLOW).mean()
    )
    hist = (macd - macd.ewm(span=MACD_SIG, adjust=False, min_periods=MACD_SIG).mean()).to_numpy(
        float
    )
    ema_up = ema > np.roll(ema, 1)
    hist_up = hist > np.roll(hist, 1)
    ema_up[0] = hist_up[0] = False
    green = ema_up & hist_up
    red = (~ema_up) & (~hist_up)
    long_raw = green & ~np.roll(green, 1)
    short_raw = red & ~np.roll(red, 1)
