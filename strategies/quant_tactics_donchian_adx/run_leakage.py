...
from strategies.quant_tactics_donchian_adx.build_features import build_features


def main() -> None:
    df = load_ohlcv(research_db(), "ETHUSDT", "1h")
    if len(df) > 8000:
        df = df.iloc[-8000:].reset_index(drop=True)
    out_dir = ROOT / "artifacts" / "reports" / "quant_tactics_donchian_adx"
