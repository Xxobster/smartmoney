...
from strategies.tradingrush_bb_breakout.signals import (
    STRATEGY_ID,
    STRATEGY_VERSION,
    build_signal_frame,
    to_tradesim_signals,
...
def run_one(symbol: str, timeframe: str) -> dict:
    ...
    signals = to_tradesim_signals(feat, symbol, max_hold_bars=HOLDS.get(timeframe, 48))
    if not signals:
        return {
            "symbol": symbol,
            "timeframe": timeframe,
...
def main() -> None:
    out_dir = ROOT / "artifacts" / "reports" / "tradingrush_bb_breakout"
    out_dir.mkdir(parents=True, exist_ok=True)
    print("=== tradingrush_bb_breakout H0 ===\nMaximum earned readiness: LIVE_STOP / RESEARCH_ONLY\n")
    results = []
    for tf in ("1h", "15m"):
