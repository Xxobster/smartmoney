...
from strategies.tsm_ema200_channel_breakout.signals import (
    STRATEGY_ID,
    STRATEGY_VERSION,
    build_signal_frame,
    to_tradesim_signals,
...
def run_one(symbol: str, timeframe: str) -> dict:
    ...
    signals = to_tradesim_signals(feat, symbol, max_hold_bars=96 if timeframe == "15m" else 48)
    if not signals:
...
def main() -> None:
    out_dir = ROOT / "artifacts" / "reports" / "tsm_ema200_channel_breakout"
    out_dir.mkdir(parents=True, exist_ok=True)
    print("=== tsm_ema200_channel_breakout H0 ===")
    ...
    for tf in ("15m", "1h"):
        for sym in ("BTCUSDT", "ETHUSDT"):
            ...
            m = run_one(sym, tf)
            results.append(m)
            print(f"  n_trades={m.get('n_trades')} PF={m.get('profit_factor')} SharpeAnn={m.get('sharpe_ann')}")
