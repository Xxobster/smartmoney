...
from strategies.tsm_ema2155_ribbon.signals import (
    STRATEGY_ID,
    STRATEGY_VERSION,
    build_signal_frame,
    to_tradesim_signals,
...
def run_one(symbol: str, timeframe: str) -> dict:
    ...
    hold = 36 if timeframe == "1h" else 24
...
def main() -> None:
    out_dir = ROOT / "artifacts" / "reports" / "tsm_ema2155_ribbon"
    out_dir.mkdir(parents=True, exist_ok=True)
    print("=== tsm_ema2155_ribbon H0 ===\nMaximum earned readiness: LIVE_STOP / RESEARCH_ONLY\n")
    results = []
    for tf in ("1h", "4h"):
