...
from strategies.tsm_engulf_demand.build_features import build_features


def main() -> None:
    df = load_ohlcv(research_db(), "BTCUSDT", "4h")
    if len(df) > 8000:
        df = df.iloc[-8000:].reset_index(drop=True)
    out_dir = ROOT / "artifacts" / "reports" / "tsm_engulf_demand"
    out_dir.mkdir(parents=True, exist_ok=True)
    report = run_leakage_audit(
        ohlcv=df, build_features=build_features, interval="4h",
