...
from strategies.tsm_failed_breakout.signals import (
    STRATEGY_ID,
    STRATEGY_VERSION,
    build_signal_frame,
    to_tradesim_signals,
...
def run_one(symbol: str, timeframe: str) -> dict:
    ...
    hold = 32 if timeframe == "15m" else 24
...
def main() -> None:
    out_dir = ROOT / "artifacts" / "reports" / "tsm_failed_breakout"
    out_dir.mkdir(parents=True, exist_ok=True)
    print("=== tsm_failed_breakout H0 ===\nMaximum earned readiness: LIVE_STOP / RESEARCH_ONLY\n")
    results = []
    for tf in ("15m", "1h"):
