...
from strategies.tsm_heikin_ashi_ema50.build_features import build_features
...
def main() -> None:
    ...
    out_dir = ROOT / "artifacts" / "reports" / "tsm_heikin_ashi_ema50"
    ...
    summary = {"ok": bool(report.ok), "summary": report.summary(), "interval": "15m"}
    (out_dir / "leakage_h0.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    (ROOT / "strategies" / "tsm_heikin_ashi_ema50" / "LEAKAGE.md").write_text(
        f"# Leakage\n\n- **ok:** `{summary['ok']}`\n\n```\n{summary['summary']}\n```\n",
        encoding="utf-8",
    )
