...
from strategies.tsm_hilo_ha_adx.signals import (
    STRATEGY_ID,
    STRATEGY_VERSION,
    build_signal_frame,
    to_tradesim_signals,
...
def run_one(symbol: str, timeframe: str) -> dict:
    ...
    hold = 48 if timeframe == "5m" else 32
...
def main() -> None:
    out_dir = ROOT / "artifacts" / "reports" / "tsm_hilo_ha_adx"
    out_dir.mkdir(parents=True, exist_ok=True)
    print("=== tsm_hilo_ha_adx H0 ===\nMaximum earned readiness: LIVE_STOP / RESEARCH_ONLY\n")
    results = []
    for tf in ("5m", "15m"):
        for sym in ("BTCUSDT", "ETHUSDT"):
            ...
            m = run_one(sym, tf)
            results.append(m)
            print(f"  n_trades={m.get('n_trades')} PF={m.get('profit_factor')} WR={m.get('win_rate')} status={m.get('status')}")
