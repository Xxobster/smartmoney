...
from strategies.tsm_impulse_volume.signals import (
    STRATEGY_ID,
    STRATEGY_VERSION,
    build_signal_frame,
    to_tradesim_signals,
...
def run_one(symbol: str, timeframe: str) -> dict:
    ...
    signals = to_tradesim_signals(feat, symbol, max_hold_bars=36 if timeframe == "1h" else 24)
    if not signals:
...
def main() -> None:
    out_dir = ROOT / "artifacts" / "reports" / "tsm_impulse_volume"
    out_dir.mkdir(parents=True, exist_ok=True)
    print("=== tsm_impulse_volume H0 ===\nMaximum earned readiness: LIVE_STOP / RESEARCH_ONLY\n")
    results = []
    for tf in ("1h", "4h"):
