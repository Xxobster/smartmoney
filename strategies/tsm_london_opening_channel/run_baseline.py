...
from strategies.tsm_london_opening_channel.signals import (
    STRATEGY_ID,
    STRATEGY_VERSION,
    build_signal_frame,
    to_tradesim_signals,
...
def run_one(symbol: str, timeframe: str = "1h") -> dict:
    db = research_db()
    ...
    signals = to_tradesim_signals(feat, symbol, max_hold_bars=24)
    if not signals:
...
def main() -> None:
    out_dir = ROOT / "artifacts" / "reports" / "tsm_london_opening_channel"
    out_dir.mkdir(parents=True, exist_ok=True)
    print("=== tsm_london_opening_channel H0 ===")
    ...
    for sym in ("BTCUSDT", "ETHUSDT"):
        ...
        m = run_one(sym, "1h")
        results.append(m)
        print(f"  n_trades={m.get('n_trades')} PF={m.get('profit_factor')} SharpeAnn={m.get('sharpe_ann')} status={m.get('status')}")
