...
from strategies.tsm_london_opening_channel.build_features import build_features
...
def main() -> None:
    ...
    summary = {"ok": bool(report.ok), "summary": report.summary(), "interval": "1h"}
    (out_dir / "leakage_h0.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    (ROOT / "strategies" / "tsm_london_opening_channel" / "LEAKAGE.md").write_text(
        f"# Leakage\n\n- **ok:** `{summary['ok']}`\n\n```\n{summary['summary']}\n```\n",
        encoding="utf-8",
    )
