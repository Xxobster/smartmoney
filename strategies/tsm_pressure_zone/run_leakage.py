...
from strategies.tsm_pressure_zone.build_features import build_features
...
def main() -> None:
    ...
    out_dir = ROOT / "artifacts" / "reports" / "tsm_pressure_zone"
    ...
    (ROOT / "strategies" / "tsm_pressure_zone" / "LEAKAGE.md").write_text(
        f"# Leakage\n\n- **ok:** `{summary['ok']}`\n\n```\n{summary['summary']}\n```\n",
        encoding="utf-8",
    )
