...
from strategies.tsm_rsi_adx_ema_scalp.build_features import build_features


def main() -> None:
    df = load_ohlcv(research_db(), "BTCUSDT", "5m")
    if df.empty:
    ...
    out_dir = ROOT / "artifacts" / "reports" / "tsm_rsi_adx_ema_scalp"
    out_dir.mkdir(parents=True, exist_ok=True)
    report = run_leakage_audit(
        ohlcv=df, build_features=build_features, interval=interval,
        registry_path=str(out_dir / "leakage_registry.json"),
    )
    summary = {"ok": bool(report.ok), "summary": report.summary(), "interval": interval}
    (out_dir / "leakage_h0.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    (ROOT / "strategies" / "tsm_rsi_adx_ema_scalp" / "LEAKAGE.md").write_text(
        f"# Leakage\n\n- **ok:** `{summary['ok']}`\n- **interval:** {interval}\n\n```\n{summary['summary']}\n```\n",
        encoding="utf-8",
    )
