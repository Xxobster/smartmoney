...
from strategies.tsm_triple_ema_pullback.build_features import build_features
...
def main() -> None:
    ...
    (ROOT / "strategies" / "tsm_triple_ema_pullback" / "LEAKAGE.md").write_text(
        f"# Leakage — tsm_triple_ema_pullback\n\n- **ok:** `{summary['ok']}`\n\n```\n{summary['summary']}\n```\n",
        encoding="utf-8",
    )
