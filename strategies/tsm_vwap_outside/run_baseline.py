...
from strategies.tsm_vwap_outside.signals import (
    STRATEGY_ID,
    STRATEGY_VERSION,
    build_signal_frame,
    to_tradesim_signals,
...
def run_one(symbol: str, timeframe: str = "15m") -> dict:
    db = research_db()
    ...
    htf = load_ohlcv(db, symbol, "1h")
    touch = load_ohlcv(db, symbol, "1m")
    feat = build_signal_frame(bars_df, htf_1h=htf)
    signals = to_tradesim_signals(feat, symbol, max_hold_bars=32)
    if not signals:
...
def main() -> None:
    out_dir = ROOT / "artifacts" / "reports" / "tsm_vwap_outside"
    out_dir.mkdir(parents=True, exist_ok=True)
    print("=== tsm_vwap_outside H0 ===\nMaximum earned readiness: LIVE_STOP / RESEARCH_ONLY\n")
    results = []
