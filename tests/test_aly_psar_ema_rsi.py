"""Tests for Parabolic Stop and Reverse + EMA + RSI H0."""
...
from strategies.aly_psar_ema_rsi.signals import WARMUP, build_signal_frame, to_tradesim_signals
...
def test_truncation_stable():
    ...
    trunc = build_signal_frame(df.iloc[:240].reset_index(drop=True))
    assert np.array_equal(
        full["long_signal"].iloc[:240].to_numpy(),
