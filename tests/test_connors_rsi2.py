"""Tests for Connors 2-period Relative Strength Index H0."""
...
from strategies.connors_rsi2.signals import WARMUP, build_signal_frame, to_tradesim_signals
...
def test_truncation_stable():
    ...
    assert np.allclose(
        full["rsi2"].iloc[:300].to_numpy(float),
        trunc["rsi2"].to_numpy(float),
        equal_nan=True,
    )
