"""Tests for HalfTrend + Exponential Moving Average 60 H0."""
...
from strategies.halftrend_ema_atr.signals import WARMUP, build_signal_frame, to_tradesim_signals
...
def test_truncation_stable():
    ...
    assert np.array_equal(
        full["ht_trend"].iloc[:300].to_numpy(),
        trunc["ht_trend"].to_numpy(),
    )
