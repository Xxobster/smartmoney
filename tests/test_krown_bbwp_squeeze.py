"""Tests for Krown Bollinger Band Width Percentile squeeze H0."""
...
from strategies.krown_bbwp_squeeze.signals import WARMUP, build_signal_frame, to_tradesim_signals


def _synthetic(n: int = 500) -> pd.DataFrame:
    rng = np.random.default_rng(7)
    close = 100.0 + np.cumsum(rng.normal(0.02, 0.6, size=n))
...
def test_sides_separated():
    feat = build_signal_frame(_synthetic(600))
