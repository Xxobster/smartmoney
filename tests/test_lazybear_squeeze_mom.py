"""Tests for LazyBear Squeeze Momentum H0 pack."""
...
from strategies.lazybear_squeeze_mom.signals import WARMUP, build_signal_frame, to_tradesim_signals
...
def test_truncation_stable():
    ...
    assert np.allclose(
        full["sqz_mom"].iloc[:300].to_numpy(float),
        trunc["sqz_mom"].to_numpy(float),
        equal_nan=True,
    )
