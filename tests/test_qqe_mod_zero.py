"""Tests for QQE MOD zero-line H0 pack."""
...
from strategies.qqe_mod_zero.signals import WARMUP, build_signal_frame, to_tradesim_signals
...
def test_sides_separated():
    feat = build_signal_frame(_synthetic(420))
...
def test_truncation_stable():
    ...
    assert np.allclose(
        full["qqe_zero"].iloc[:300].to_numpy(float),
        trunc["qqe_zero"].to_numpy(float),
        equal_nan=True,
    )
