"""Tests for SuperTrend + EMA 200 H0."""
...
from strategies.quant_tactics_st_ema200.signals import WARMUP, build_signal_frame, to_tradesim_signals
...
def test_sides_separated():
    feat = build_signal_frame(_synthetic(360))
...
def test_truncation_stable():
    ...
    assert np.allclose(
        full["st_line"].iloc[:300].to_numpy(float),
        trunc["st_line"].to_numpy(float),
        equal_nan=True,
    )
